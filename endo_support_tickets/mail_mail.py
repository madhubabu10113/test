...
class mail_mail(osv.Model):
    ...
    def send(self, cr, uid, ids, auto_commit=False, recipient_ids=None, context=None):
        mail = super(mail_mail, self).send(cr, uid, ids, auto_commit, recipient_ids, context=context)
        context = context or {}
        if mail and context.get('default_model') == 'crm.claim' and context.get('default_res_id') and context.get('mark_ticket_as_acknowledged'):
            self.pool.get('crm.claim').write(cr, uid, [context.get('default_res_id')], {'x_acknowledged':True,},context=None)
        return mail
        ...
        if partner:
            ...
            if partner.user_ids and mail.model and mail.res_id \
                ...
                try:
                    ...
                    text = _("""<p>Access this document <a href="%s">directly in Endosoft ERP</a></p>""") % url
                except except_orm, e:
                    pass
